...
const layoutId = "3080264545"; // Tallytop Layout v2
...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} Tallytop Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} Tallytop's recommended Steam Deck controller settings?
